...
tenant_registration = folder(
    "7. Tenant Registration",
    "CR-028's public signup, plus CR-062's two availability lookups. All three "
    "are unauthenticated (permitAll) and rate-limited. The availability "
    "endpoints exist so the signup wizard can reject a taken shop name or "
    "mobile number at the step that asks for it, rather than after the plan "
    "and Terms steps - see CR-062. They are a deliberate, documented "
    "enumeration surface governed by REGISTRATION_AVAILABILITY_PER_IP "
    "(20/min/IP), shared across both.",
    [
        request(
            "Shop name available", "GET", "/v1/tenants/register/slug-available",
            auth=False, params=[{"key": "slug", "value": "A Brand New Shop"}],
            desc="Live-checked while the owner types the shop name.",
            tests=ENVELOPE_OK + [
                "pm.test('Reports availability', function () {",
                "    pm.expect(pm.response.json().data.available).to.be.a('boolean');",
                "});",
            ]),

        request(
            "Shop name taken", "GET", "/v1/tenants/register/slug-available",
            auth=False, params=[{"key": "slug", "value": "Default"}],
            desc="V6 seeds the 'default' tenant in every install, so this name is reliably taken anywhere the collection runs. (An earlier draft used the seeded shop's display name, which is renameable - and had in fact been renamed on the dev box, so the assertion was already false.)",
            tests=[
                "pm.test('Status is 200', () => pm.response.to.have.status(200));",
                "pm.test('Seeded shop name is not available', function () {",
                "    pm.expect(pm.response.json().data.available).to.eql(false);",
                "});",
            ]),

        request(
            "Mobile and email available", "GET",
            "/v1/tenants/register/identifier-available", auth=False,
            params=[{"key": "mobileNo", "value": "9000000999"},
                    {"key": "email", "value": "availability.probe@example.in"}],
            desc="CR-062. Both params are optional; an omitted one always reports available, so no verdict is volunteered about a value the caller never sent. These two values are deliberately NOT the ones the register request below uses, so running the whole folder does not make this assertion fail on the second pass.",
            tests=ENVELOPE_OK + [
                "pm.test('Both identifiers free', function () {",
                "    const d = pm.response.json().data;",
                "    pm.expect(d.mobileAvailable).to.eql(true);",
                "    pm.expect(d.emailAvailable).to.eql(true);",
                "});",
            ]),

        request(
            "Mobile already registered", "GET",
            "/v1/tenants/register/identifier-available", auth=False,
            params=[{"key": "mobileNo", "value": "9876543210"}],
            desc="The seeded OWNER's number. This is the case that used to "
                 "sail through the wizard and only fail on the final Create "
                 "account call, after the Terms had been accepted.",
            tests=[
                "pm.test('Status is 200', () => pm.response.to.have.status(200));",
                "pm.test('Seeded owner mobile is not available', function () {",
                "    pm.expect(pm.response.json().data.mobileAvailable).to.eql(false);",
                "});",
            ]),

        request(
            "Register a new shop", "POST", "/v1/tenants/register", auth=False,
            body={
                "shopName": "Postman Test Hardware",
                "ownerFullName": "Postman Owner",
                "mobileNo": "9000000123",
                "email": "brand.new@example.in",
                "password": "Passw0rd",
            ...
            desc="Creates a tenant, its role set and its first OWNER. Rerunning "
                 "this returns 409 on the duplicate mobile - which is exactly "
                 "the late failure CR-062's availability check now surfaces "
                 "three steps earlier.",
            tests=[
                "pm.test('Created, or already exists from a previous run', function () {",
                "    pm.expect(pm.response.code).to.be.oneOf([201, 409]);",
                "});",
            ]),

        request(
            "BUG-SEC-005: availability lookups are rate limited", "GET",
            "/v1/tenants/register/slug-available", auth=False,
            params=[{"key": "slug", "value": "Enumeration Probe"}],
            desc="Run with Postman's Runner (iterations: 30) under the default "
                 "profile. Until CR-062 this endpoint had NO limit at all: "
                 "RateLimitFilter dispatches on exact path equality and "
                 "'/v1/tenants/register/slug-available' is not equal to "
                 "'/v1/tenants/register', so it fell through ungoverned next to "
                 "a sibling metered at 5/hour. It now shares a 20/min/IP bucket "
                 "with identifier-available.",
            tests=[
                "pm.test('Either answered or rate limited', function () {",
                "    pm.expect(pm.response.code).to.be.oneOf([200, 429]);",
                "});",
                "if (pm.response.code === 429) {",
                "    pm.test('Code is RATE_LIMIT_EXCEEDED', function () {",
