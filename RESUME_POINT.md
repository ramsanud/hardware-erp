...
**Why not a CAPTCHA alone, which is what was literally asked for.** `CaptchaProperties.active()` treats a missing key as disabled — deliberately, so an unconfigured install does not lock everyone out. A CAPTCHA-only gate would therefore be **no gate at all** on exactly those installs, while the dialog still claimed a security check. Turnstile is also usually invisible, so on its own it proves *a human*, never *this human meant it*. The typed shop name is the half that configuration cannot switch off. This was raised, and the owner chose that pairing.

**Backend.** `V54__data_reset_permission.sql` seeds `DATA_RESET` (module `SETTINGS`, OWNER only) — a code of its own, never `SETTINGS_MANAGE`, so granting settings access never hands over a wipe button. `DataResetService`/`Impl` + `DataResetController` at `GET /v1/settings/data-reset/preview` and `POST /v1/settings/data-reset`. 27 deletes in explicit FK order, one transaction, every one scoped by `requireCurrentTenantId()`; no endpoint, field or parameter in the feature accepts a tenant id or reaches a table name.

**Two follow-on effects found by tracing the workflow, not in the ask, fixed here.** `coupon.times_used` is a counter on a *surviving* master that counts redemptions against *deleted* invoices — a coupon with `usage_limit = 100` would stay permanently spent; reset to 0. And `document_sequence` would keep issuing INV-0341 into an empty list; restarted at 1 for `INVOICE`/`QUOTATION`/`PURCHASE`/`PROJECT` only, never for the master types whose records survive.

**`credit_note_item` holds an FK to `invoice_item`**, so credit notes must be deleted before invoices. That is the one ordering a plain `ON DELETE CASCADE` would not have arranged, and the reason all 27 statements are written out rather than leaning on the cascades most of them do have — that, and a cascade reporting no row count, which would leave the confirmation dialog unable to say honestly what it removed.

**Frontend.** `DataResetCard` in Shop Settings, rendered only for a `DATA_RESET` holder, opening a dialog that loads live counts ("342 invoices", not "your data" — the same preview-then-commit shape the bill and product imports already use), states plainly what is kept, requires the typed shop name, and renders the Turnstile widget when one is configured. **It deliberately does not reload on success** — the summary of what was deleted is the only place the owner will ever see it, and an immediate reload would wipe it, which is exactly the bug CR-032 had to fix twice.

**Verified — executed, not claimed.** `DataResetIT` 6/6 against real PostgreSQL, including a second throwaway shop asserted untouched afterwards (that test exists specifically because seven child tables carry no `tenant_id` and are reached by subquery). `mvn -o clean verify` — see the run recorded below. `tsc -b --force` and `vite build` both exit 0. `registry/static_check.py` **not executed** — `python3` is not installed on this machine.

**The IT registers two throwaway shops per test instead of resetting tenant 1.** `AbstractIntegrationTest` shares one reused container with no rollback between methods, so a reset aimed at the seeded tenant would have stripped the products, stock and movements every other IT reads, and the failures would have surfaced somewhere unrelated.

**Not done, and not started.** Password re-entry as a third factor (offered, declined). A CAPTCHA on the other 47 `ConfirmDialog` sites (offered, declined — a challenge on "delete this brand" is trained away in a week, and a control that has been trained away is worse than none because the screen still claims one). No Postman entry was added: the only collection in `docs/` covers Module 1 auth and contains no `/v1/settings` request at all, so there was nothing to extend.

**⚠ Other Claude sessions are writing to this tree.** `git status` spans more than one session's work — check and split before committing.

---


**Updated:** 2026-09-07 (**stabilization pass — CR-064, CR-065, BUG-BE-001, BUG-DB-001, BUG-SEC-006, BUG-FE-032, and the first frontend test runner**). Defect and security work only; no new ERP features, no module redesigns, no API contract changes beyond what CR-065's transport required.

**P0-1, project material → stock (CR-064 / BUG-BE-001).** `ProjectMaterialServiceImpl` wrote `project_material` and never touched `stock_movement`, so `quantity_on_hand` stayed overstated by everything every project had ever consumed. Fixed by calling the existing `StockService.applyMovement` from add/update/remove — **no second stock calculation was introduced**, because that method already locks the row, takes the tenant from the JWT, refuses a negative balance and joins the caller's transaction. Editing writes the DIFFERENCE (15 → 20 takes a further 5, not another 20); swapping the product reverses the old in full then consumes the new, reversal first.

**Two business decisions were put back to the owner rather than guessed**, and both are recorded in CR-064 with their reasoning. (1) `quantityActual` alone moves stock, with `quantityWastage` treated as a breakdown *within* it — matching `totalCost()`, which prices actual and ignores wastage. **If that reading ever flips, `totalCost()` has to flip in the same commit or the project is under-billed.** (2) Cancelling a project does **not** restore stock: materials used on a half-built job have not come back, and saying they have would re-create the very overstatement this CR fixes. Removing an individual material row *does* reverse, because that is a correction of a record rather than a claim about the yard.

**A latent defect found while doing it — BUG-DB-001, and it is the more interesting one.** `stock_movement.movement_type` has been `VARCHAR(20)` since V8, and its CHECK constraint was extended in V21, V36 and V37 without the column ever being widened. **V37 added `SALES_RETURN_REVERSAL`, which is 21 characters** — the constraint permits a value the column cannot store, so cancelling a credit note would have died on a Postgres length error. Latent only because Credit Note has no frontend. `ddl-auto: validate` does not catch this: it checks the column exists and is a string, not that the entity's `length` and the CHECK agree. Found by counting characters, not by a test. V53 widens to 30.

**P0-2, Platform Admin refresh token (CR-065 / BUG-SEC-006).** It was returned in the JSON body and held in a module variable, so any XSS on that origin could read a 7-day credential for a console that suspends tenants and touches billing — and a reload signed the admin out. **Transport only**: rotation, replay-as-theft and `token_version` were already correct and are untouched, and the new IT re-asserts them *through* the cookie specifically to prove that. New `PlatformAdminRefreshTokenCookieService` is a sibling of the tenant one, differing only in cookie **name** (`erp_pa_refresh_token` — so a staff session and a shop session in one browser cannot overwrite each other) and **path** (`/api/v1/platform-admin/auth` — so a staff credential is never attached to a tenant request). **CSRF was not weakened**: `SameSite=Strict` + path scope is the existing justification for `csrf.disable()`, and the cookie was configured so that stays true. The reload fix is the provider's new bootstrap-on-mount, not a more durable access token.

**Verified — actually executed, with Docker up (BUG-ENV-002 reasoning is obsolete and was not used).** `ProjectMaterialStockTest` 12/12; `ProjectMaterialStockIT` 5/5 against real PostgreSQL, including 16 concurrent consumers against a balance of 10 (exactly 10 succeed, 6 refused, balance never negative); `PlatformAdminRefreshCookieIT` 11/11; `PlatformAdminRefreshTokenCookieServiceTest` 6/6; `tsc -b --force` and `vite build` exit 0; the new frontend suite 60/60.

**P1-2 — the frontend now has a test runner, in the repo: `frontend/tests/`, `npm run test:e2e`.** Deliberately ~120 lines of harness driving Playwright, not a framework: this project had none at all and every BUG_REGISTRY entry said "no regression test". Three suites (auth, products, navigation) covering BUG-FE-024/025/026/028 and 25 routes × 5 viewports. **It runs against `vite preview`, i.e. the production bundle** — two of the defects it covers live in generated CSS and a dev-server run would not exercise the file that ships. `playwright` was in `node_modules` but **not declared in package.json**; it is now a devDependency, because an undeclared one vanishes on a fresh `npm ci` and takes the suite with it. Nothing new was installed.

**Two measurement traps are documented in `frontend/tests/README.md` and cost real time here.** Playwright's `boundingBox()` returns DOCUMENT coordinates under mobile emulation — it made a correctly positioned fixed bottom sheet look 400px below the fold and produced four confident false failures; use `getBoundingClientRect()` inside `page.evaluate`. And Playwright matches routes in REVERSE registration order, so a broad `**/v1/auth/**` stub silently shadows a specific `/refresh` one and the app never signs in.

**Deliberately not done, and why.** Filtering the Users list by `mustChangePassword` (the status filter is a server-side query parameter, so it needs a backend query change — an API change inside a stabilization pass). And the five items already deferred under CR-063: dialog back-button history, URL-synced list filters, the systemic `FormField` aria-describedby fix, `DialogDescription` across 29 dialogs, clear buttons on every module form.

**One thing to look at that this pass did not change.** `ProjectMaterialRequest.quantityWastage` is documented in its `@Schema` as *"Extra quantity lost to cutting, handling or damage"* — "extra" reads as *additional to* actual, which is the opposite of the semantics CR-064 just pinned. The behaviour is what the owner chose; the annotation is now misleading and should be reworded, or the decision revisited. Flagged, not silently changed.

**A flaky first full run, and it was this pass's fault — worth knowing before writing more ITs.** `mvn -o verify` failed once with 1 failure and 11 errors, then passed twice cleanly (472 unit + 209 integration, BUILD SUCCESS both times). The difference was container state: `AbstractIntegrationTest` declares `withReuse(true)`, and the first run inherited rows left by the targeted IT runs done while building this change. **`ProjectMaterialStockIT` was creating a fresh tenant, role and user on every execution**, so on any machine with Testcontainers reuse enabled it would have accumulated a shop per run until a neighbouring test that counts tenants failed for reasons nobody could trace back here. It now reuses a fixed slug (`it-tenant-isolation`) and costs at most one extra tenant, ever. **An IT that writes rows a later test can see must be idempotent, not merely correct.**

**⚠ At least two Claude sessions are writing to this tree, and one committed mid-pass.** `BUG-FE-031` was already taken by another session's profile-overflow fix when this pass tried to use it — renumbered to `BUG-FE-032` rather than overwriting. Partway through, that session committed `ac86085` and `6f78d98`, which swept up this pass's already-edited `platformAdminApiClient.ts` and `platformAdminTokenStorage.ts` along with its own work; the CR-065 changes in them were verified intact afterwards (`withCredentials`, no `getRefreshToken`, provider bootstrap) and the full suite re-run green on top. `git status` still spans more than one session's work. **Check it before committing, and split.**

---

**Updated:** 2026-09-07 (**full-application UX audit → CR-063 + BUG-FE-024…030**). The ask was a complete audit first, fixes second, tests third — with two named findings (stale state returning to Login; no clear buttons) and three named UI targets (product status colours, the phone product grid, dialog scrolling).

**FRONTEND ONLY. No Java file, DTO, migration, or backend test was touched, and no API contract changed.** `mvn clean verify` was therefore **not executed** — there is nothing in this change for it to cover. `registry/static_check.py` **not executed** (`python3` is not installed here).

**The reported "goes back to Reset Password" bug had two root causes, and neither was the one the symptom suggested.** (1) `AuthProvider.mfaToken` survived a user backing out of the second factor, so Forward re-entered an abandoned sign-in — fixed with `cancelPendingLogin()`, called by `LoginPage` on mount, on the principle that *arriving at the sign-in screen is the user saying "start again"*. (2) `ResetPasswordPage` read its token from the URL on every render, so `/reset-password?token=…` stayed in the history stack — Back from `/login` re-entered the form with a token the server had already burned, and the token sat in history and in every later `Referer`. Fixed by lifting it into a ref and rewriting the URL with `replace:true`. See BUG-FE-025.

**The worst bug found was not reported and is Critical: BUG-FE-027.** Completing a forced password change signs every session out *while the user is on `/change-password`*, so `ProtectedRoute` recorded that path as `state.from`, and the next sign-in redirected straight back to the change-password screen — with `mustChangePassword` now `false`, nothing bounced them out. A new owner-created user could not reach the dashboard without editing the URL. Fixed in both `ProtectedRoute` (never record an auth route) and `LoginPage` (refuse one that arrives anyway, since `from` is history state and survives a Back into that entry).

**BUG-FE-028 is BUG-FE-023's twin and is the lesson worth carrying.** CR-061 fixed a 16px strip of scrolling form showing *below* the pinned footer. The identical defect existed *above* the pinned header, for the identical reason — `position:sticky` pins to the scroll container's padding edge, and no margin moves it past that — and survived because nobody scrolled a dialog and looked at the top edge. **When a fix turns on a general fact about the layout system, check every element that fact applies to.**

**Verified — actually executed, in Chromium via Playwright against the production build:** 27/27 auth-flow assertions (clear buttons, show/hide, 40×40 touch targets measured not eyeballed, forgot→back→login, token stripping, Back never restoring a tokened URL); 31/31 product-grid and dialog assertions (Active `rgb(24,140,66)` / Inactive `rgb(197,32,32)`, price present on the phone card, sticky header/footer measured as immobile while the body scrolls 500px, header occluding the panel's top pixel row); 15/15 responsive-regression assertions across **25 routes × 5 viewports (320/414/768/1280/1920)** — every route renders, zero horizontal overflow, zero uncaught errors. `tsc -b --force` and `vite build` both exit 0.

**Two measurement traps worth knowing before writing more browser tests here.** (1) Playwright's `boundingBox()` returns **document** coordinates under mobile emulation, which made a correctly positioned fixed bottom sheet look as though it hung 400px below the fold — four "failures" that were the test's fault, not the app's. Use `getBoundingClientRect()` inside `page.evaluate` for anything `position:fixed`. (2) Playwright matches routes in **reverse registration order**, so a broad `**/v1/auth/**` stub silently shadows a specific `/refresh` one and the app never signs in.

**The test harness is deliberately NOT in the repo.** It lives in this session's scratchpad (`qa/auth-flows.spec.mjs`, `qa/product-grid-and-dialogs.spec.mjs`, `qa/responsive-regression.spec.mjs`). `playwright` is already in `node_modules`; adopting it as the project's first frontend test runner is a real decision with a real maintenance cost, and CLAUDE.md says not to stand up a subsystem unprompted. **It is the single highest-value follow-up** — every fix in this pass is recorded in `BUG_REGISTRY.md` as "no regression test", and that is now nine entries deep.

**Two things asked for and deliberately not built, both recorded under CR-063 "Explicitly rejected".** Back-button-closes-dialog (needs a history entry per dialog across 30+ dialogs, some nested, with no test runner to catch the double-pop cases) and URL-synced list filters (worth doing to all 15 list pages at once, not half-way inside a bug-fix pass). Clear buttons are on the auth screens; the remaining module forms are listed as follow-up, not swept blindly, because "clear this field" is wrong on some of them.

**⚠ At least two Claude sessions have been writing to this tree.** The 2026-09-05 entry below lists `clearable-input.tsx`, `StatusBadge.tsx`, `PasswordFields.tsx`, `badge.tsx`, `ConfirmDialog.tsx` and `ProductListPage.tsx` as "none of them mine" — those are **this** session's files, and `RESUME_POINT.md` was written 1.5 hours after they were created. Nothing was clobbered in either direction as far as `tsc`/`vite`/the browser suites can tell, but **`git status` shows more than one session's work and the commit should be split accordingly.**

---

**Updated:** 2026-09-05 (**CR-062 — auth screens simplified; signup checks the mobile number at the step that asks for it; mobile "More" menu**). Three asks: refactor login/signup ("too absurd and too complex"), validate the mobile number on Next rather than the final button, and fix the "More" option that "not looks good in sidebar".

**The signup diagnosis was not what the report literally said, and that matters.** Format validation *already* ran on Next — reproduced live, a malformed number is caught there. What sailed through was a **well-formed but already-registered** number: tested against the seeded owner's `9876543210`, the wizard advanced to step 3, and the duplicate only surfaced from `POST /v1/tenants/register` after the user had chosen a plan and accepted the Terms, then bounced them back. The old code even documented that bounce as intended. So the fix was a uniqueness check, not a format one.

**Built (SPECIFICATION CHANGE):** `GET /v1/tenants/register/identifier-available`, public, returns `{mobileAvailable, emailAvailable}`, called from the wizard's Next. The service method reuses the very same `existsByMobileNo`/`existsByEmailIgnoreCase` calls `register()` makes, so the wizard's "free" and the create call's "taken" can never disagree. The frontend **fails open** on a lookup error — a wizard nobody can advance would be worse than the old behaviour, and the server still rejects duplicates regardless.

**The enumeration trade-off was put to the user before building and accepted.** The endpoint confirms platform-wide whether an identifier has an account (identifiers are globally unique across tenants, CR-016). It exposes no new fact — `POST /register` already answered it to anyone willing to submit — but it makes probing cheap, so the control is the rate limit. Recorded in `SECURITY_REGISTRY.md` with the conditions that would change the judgement.

**BUG-SEC-005, found while wiring that up.** `slug-available` **had no rate limit at all.** `RateLimitFilter` dispatches on exact path equality, and `/v1/tenants/register/slug-available` is not `equals` to `/v1/tenants/register`, so it fell through the `default` arm ungoverned next to a carefully-metered sibling. Fixed under the same root cause. **The structural weakness remains and is the more valuable fix**: any future endpoint added beneath one of those prefixes will be ungoverned by default, silently. Prefix matching, or a test asserting every permitAll path is either limited or explicitly exempted, would close the class — recorded, not attempted.

**Verified — actually executed:** `mvn -o verify` → **454 unit + 192 integration, BUILD SUCCESS**; `tsc -b --force` and `vite build` clean; and all three items driven in a real browser against the running local stack — brand panel measured at **27% of viewport** (was ~50%), a taken mobile **blocked on Next** with the message on the field while a free one advances, and the More menu confirmed rising from the bottom edge, full width, 26 destinations, navigating and closing correctly. **Not executed:** `registry/static_check.py` (`python3` is not installed here).

**Two things to know about this session's environment.** (1) The backend must be **restarted** to pick up the new endpoint — the first verification run failed because a backend started earlier was still serving, the lookup 404'd, and the fail-open path silently let the wizard advance. That is the fail-open working as designed, but it makes a stale backend look like a broken feature. (2) **Another Claude session was editing this same working tree throughout** (`clearable-input.tsx`, `PasswordFields.tsx`, `badge.tsx`, `StatusBadge.tsx`, `ConfirmDialog.tsx`, `ProductListPage.tsx` — none of them mine). Its mid-edit state broke the dev server's module graph with a stale-export error that looked like my breakage and was not; a Vite restart cleared it. **Check `git status` before committing — the tree contains at least two sessions' work.**

**Not done, and it should be:** no Postman collection covers tenant registration at all (`docs/` has only module-01 auth), so the new endpoint has no collection entry. CLAUDE.md's spec-change checklist asks for one.

---

**Updated:** 2026-09-05 (**CR-061 — mobile application UI**). The ask was "change the mobile ui to mobile application ui, now the ui looks absurd in mobile view, and the theme colours not adaptable for mobile". **FRONTEND ONLY** — no Java file, DTO, migration or backend test was touched, and no API contract changed.

**Scope was confirmed with the user first**, because "mobile application" has two readings that differ by days of work. Chosen: **app feel, no install** — no manifest, no service worker, no offline/IndexedDB layer, so `CLAUDE.md`'s "no PWA surface" stays deliberate and true. Chosen tabs: **Home / Invoices / Products / Customers / More**.

**Built:** a bottom tab bar below `lg` (`layouts/MobileTabBar.tsx`) whose tabs are a *priority list* filtered by permission, so a Purchase Staff user gets four working tabs rather than one and three gaps; a 56px app bar with the notch inset as padding; search promoted to a destination on a phone (`GlobalSearch` was `hidden sm:block`, i.e. **phones had no search at all**); every table re-flowing into a stacked list below `sm`; dialogs becoming bottom sheets; and `<meta name="theme-color">` synced to the painted palette — the literal cause of "theme colours not adaptable", since there was no such tag and the phone browser painted its own chrome regardless of which of the eight presets was active.

**The one change worth understanding before touching tables again:** column headings register themselves by index on `Table` and cells read theirs back out (`shared/components/ui/table.tsx`). That is what let the stacked list reach **all 39 table call sites** without asking any of them to repeat their headers — which would have drifted the first time a header was reworded. `label` on a cell overrides a wrong derivation; `mobileCards={false}` opts a table out entirely. In the CSS, `td:not(.hidden)` is load-bearing: pages already mark optional columns `hidden sm:table-cell`, and a blanket `display:flex` would have un-hidden exactly the columns each page had decided a phone should not see.

**Verified — actually executed:** `tsc -b --force` exit 0; `vite build` exit 0; and the result was **rendered under Playwright at 390×844** (touch, 2× DPR) against a stubbed API in **both** light and dark — zero horizontal overflow, zero console errors, the sheet flush to the bottom edge, and `theme-color` reading `rgb(249,250,251)` light / `rgb(12,19,34)` dark with the static tags retired. **Not executed:** `registry/static_check.py` (`python3` is not installed on this machine).

**BUG-FE-023, found and fixed in the same pass.** Every dialog showed a 16px strip of the scrolling form below its pinned Save/Cancel bar. **Pre-existing at every screen size**, not mobile-specific — the phone sheet, flush to the screen edge, is only what made it visible. Two faults stacked: the footer's `-mb-4` was cancelled outright by the `<form class="space-y-4">` nearly every dialog wraps it in (`.space-y-4 > :not([hidden]) ~ :not([hidden])` scores 0-3-0, since each `:not([hidden])` adds an attribute selector), **and** even unopposed it could not have worked, because a sticky element is pinned to its scroll container's padding edge and no margin moves it past that — confirmed by forcing the margin and watching the rendered position not move. Fixed by having `.dialog-scroll` drop its own bottom padding via `:has(.dialog-sticky-footer)`.

**Two registry gaps were noticed here. One is now closed, one is still open.** (1) **STILL OPEN — CR-060 is referenced by 18 files** (optional MFA / `LoginChallengeResponse`, backend and frontend) **but has no entry in `CHANGE_REQUEST_REGISTRY.md`**; this work is numbered 061 to avoid colliding with it. (2) **CLOSED 2026-09-08.** `BUG-FE-012`/`013`/`014`/`015`/`016`/`017`/`018`/`020`/`021`/`022` were cited in source comments but absent from `BUG_REGISTRY.md`; all ten are now back-filled there from the comments the fixes left at their call sites, and the index table — which stopped at `BUG-ENV-003`, omitting 33 entries — now covers all 86. `BUG-FE-017` (line gross reconstructed from the wrong figures) was found during that pass and **has no regression test**; adding one is recommended and is recorded in its entry.

**Left deliberately undone, proposed as its own CR:** Invoice/Quotation Detail carry up to six icon-only actions in a row. On a phone that would read better as one primary button plus an overflow menu, but that is a per-page redesign rather than a shell change, so it was not pulled in unscoped.
