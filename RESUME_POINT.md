...
**Two registry gaps noticed, not actioned — someone should back-fill them.** (1) **CR-060 is referenced by 18 files** (optional MFA / `LoginChallengeResponse`, backend and frontend) **but has no entry in `CHANGE_REQUEST_REGISTRY.md`**; this work is numbered 061 to avoid colliding with it. (2) `BUG_REGISTRY.md` stops at `BUG-FE-008`, while `BUG-FE-012`/`013`/`014`/`015`/`016`/`018`/`020`/`021`/`022` are all cited in source comments — the bug registry is behind the code.
